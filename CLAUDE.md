...
- `app/retirement/page.tsx` — Retirement page server component. Fetches retirement accounts (filtered by Plaid subtype), computes projection (monthly contribution from snapshot history, 7% growth, $2M target).
- `app/retirement/retirement-view.tsx` — Retirement page client component. Shows total balance, projection card with progress ring/bar, and account list with institution logos.
...
- `components/Sidebar.tsx` — Navigation sidebar (Dashboard, Transactions, Net Worth, Retirement, Account Management). Centered round logo at top. Slide-out drawer on mobile, fixed on desktop. Uses Tailwind utilities (no custom CSS classes). Deep sea green background (`bg-sg-900`), active nav items use `bg-sg-700 text-white` pill.
- `components/SpendBreakdown.tsx` — Donut chart of expenses by category (recharts). Filters out internal transfers (shown as excluded total). Groups small categories (<2%) into "Other" and caps at 6 slices max. Shows total in donut center. Used on `/transactions` page.
- `components/DonutChart.tsx` — Reusable rounded donut chart component (recharts PieChart with `paddingAngle` and `cornerRadius`). Takes `data` (name/value/color array) and `colors` props. Used on dashboard Spending by Category widget.
...
- `/` — Home dashboard with multiple rows: (1) Summary cards row — 4 cards (Total Balance with sea green gradient, Income with trend icon, Expenses with trend icon, Net Savings) in a 4-column grid (2x2 on mobile). (2) My Cards + Monthly Trend row — hardcoded credit cards widget (Amex Gold, Capital One Venture X) with layered/stacked card design (back card peeks left, click to swap with animation, click front card to open coming-soon detail sheet) on left (3fr), weekly expense area chart on right (7fr). Cards have 3D radial gradients and box shadows. (3) Projected Retirement + Recent Transactions + Spending by Category row — retirement age card with SVG progress ring on left (2fr), 3 latest transactions with 3D row styling in middle (5fr), rounded donut chart with category legend on right (4fr). (4) Category spend pill selector + top/worst investment performer cards. AI insight card (font-body italic) with Claude orange accent at top. Time-of-day greeting (Central Time). All dashboard DB queries (transactions, snapshots, holdings, accounts) run in parallel via `Promise.all()`.
...
- `/retirement` — Retirement planning: total retirement balance hero, projected retirement age card with progress ring and progress bar toward $2M goal (assumes 29 current age, 7% annual growth, contribution rate derived from snapshot history). Lists all retirement accounts (filtered by Plaid subtype: 401k, IRA, Roth, 403b, pension, TSP, etc.) with institution logos and prettified type labels.
