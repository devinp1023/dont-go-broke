...
- `components/Sidebar.tsx` — Navigation sidebar (Home, Transactions, Net Worth, Retirement, Account Management). Slide-out drawer on mobile, fixed on desktop. Uses Tailwind utilities (no custom CSS classes). Deep sea green background (`bg-sg-900`), active nav items use `bg-sg-700 text-white` pill.
